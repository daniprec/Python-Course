...
Write-Host "Installing Python dependencies (user): PyYAML and nbformat"
python -m pip install --user pyyaml nbformat

Write-Host "Done. Pre-commit hook will run tools/sync_execute_flags.py on commit."
Write-Host "If you use a different python binary in CI, ensure that environment has nbformat and PyYAML installed."
