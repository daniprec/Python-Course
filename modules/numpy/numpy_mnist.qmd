---
title: "Application: MNIST Digit Recognition"
...
This application downloads MNIST, installs interactive notebook widgets, and uses Google Colab features. The page displays its code without executing it during website rendering.
:::
...
- A grayscale image can be represented by a two-dimensional NumPy array.
- A collection of images forms a higher-dimensional array.
- Averaging images by class creates a simple representative image for each digit.
- Array subtraction and aggregation provide a basic distance-based classifier.
- Accuracy must be measured on labeled examples that were not used to construct the model.

This is a deliberately simple classifier. Describe one reason why an average image may fail to represent all the different ways people write the same digit.
