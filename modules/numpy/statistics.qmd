---
title: "Statistics and Simulation with NumPy"
...
print("Minimum and maximum:", np.min(measurements), np.max(measurements))
```

For a two-dimensional dataset, the `axis` argument decides whether NumPy summarizes columns or rows.
...
print("Mean of each subject:", np.mean(scores, axis=0))
print("Mean of each student:", np.mean(scores, axis=1))
```

::: {.callout-note}
## Exercise

Before running the code, predict the shapes of both mean arrays. Then compute the standard deviation of each subject.
:::

::: {.callout-tip collapse="true"}
## Solution

Both results have shape `(3,)` because the three subject columns remain after reducing axis `0`.
...
The matrices are symmetric. Their off-diagonal values describe the relationship between the two different variables.
...
This p-value is a simulated probability under the fair-coin model. It is not the probability that the hypothesis itself is true.
...
The Boolean mean is the proportion of points inside the circle because NumPy treats `True` as `1` and `False` as `0` in this calculation.
:::
...
- Descriptive statistics summarize a dataset; `axis` controls the direction of a reduction.
- Covariance and correlation describe how pairs of variables vary together.
...
Work through the Session 17 homework exercises available here.

[Homework](homework_numpy.qmd#session-17-statistics-and-simulation){.btn .btn-primary}
