...
```{python}
ls_devices = ["emf", "temp", "noise"]
ls_rooms = ["Kitchen", "Hall"]
ls_hours = ["10pm", "11pm", "0am", "1am"]

for room in ls_rooms:
  for device in ls_devices:
    # Initialize a new list, it will contain one record
    # per hour for room and device
    ls_records_per_hour = []
    for hour in ls_hours:
      filename = f"{device}-{room}-{hour}.txt"
      new_record = np.loadtxt(filename)
      ls_records_per_hour.append(new_record)
    # Join all the records together
    full_record = np.concatenate(ls_records_per_hour)
    print(f"{device} - {room}")
    plot_recordings(full_record, units=device)
```
