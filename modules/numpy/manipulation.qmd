...
Numerical data does not always arrive in the shape a calculation needs. In this lesson, we will reorganize the same values, combine compatible arrays, and divide arrays into useful pieces.
...
def rouche_frobenius_theorem(A, b):
    # Turn vector b with shape (N,) into a column with shape (N, 1).
    B = b.reshape(-1, 1)
    Ab = np.hstack((A, B))
...
You do not need to memorize every joining and splitting function. Focus on the shape of each input, the shape you want as output, and the axis along which the operation acts.

- `reshape()` changes the arrangement of values but not their count.
- `flatten()` returns a one-dimensional copy.
- `concatenate()` joins arrays along an existing axis.
- `stack()`, `vstack()`, and `hstack()` provide common arrangements.
- `split()` requires equal pieces; `array_split()` also permits unequal pieces.

Before running any joining operation, predict the output shape. Then explain why all non-joining dimensions must match.

---

# Homework and Review

Work through the Session 14 homework exercises available here.

[Homework](homework_numpy.qmd#session-14-array-manipulation){.btn .btn-primary}
