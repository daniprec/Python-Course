---
title: "Application: Numerical Linear Algebra"
...
This session combines NumPy array operations with mathematical algorithms. Each task begins with a clear specification; attempt it before opening the example solution.
...
- Centering uses broadcasting to subtract one column mean from every row.
- Gram–Schmidt combines dot products, vector subtraction, and norms.
- Conjugate gradient solves suitable linear systems iteratively.
- The power method estimates the eigenvalue of largest magnitude and a corresponding eigenvector.

For each algorithm, identify which operations are vectorized and which steps still require a loop.
