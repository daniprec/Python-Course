---
title: "Loading and Saving NumPy Data"
...
# Loading and Saving Data: Ghost Hunting
...
2. **Placed three monitoring devices in each room:**
    ...
    - **Microphone:** Let's capture those ghostly whispers! This will measure noise levels in decibels. We know that background noise in the house is about 20 dB.
...
This lesson downloads a prepared dataset and is intended to run in Google Colab. The page displays the code without executing it during website rendering.
:::
...
Use `np.savetxt()` when the data should remain readable as plain text. A later Python session can recover the array with `np.loadtxt()`.

```{python}
hourly_activity = np.array([0, 1, 0, 2, 1])
np.savetxt("hourly_activity.txt", hourly_activity, fmt="%d")

loaded_activity = np.loadtxt("hourly_activity.txt", dtype=int)
print(loaded_activity)
```

NumPy's binary `.npy` format preserves the array's shape and data type exactly.

```{python}
np.save("hourly_activity.npy", hourly_activity)
loaded_binary = np.load("hourly_activity.npy")
print(loaded_binary)
```
...
- `np.loadtxt()` loads numerical text data into an array.
- `np.savetxt()` saves an array as readable text.
- `np.save()` and `np.load()` preserve NumPy arrays in binary form.
- `np.concatenate()` joins consecutive recordings into one array.
- Boolean comparisons can convert sensor readings into activity masks.

Explain why filenames, array shapes, and sampling order must remain consistent when several recordings are concatenated.

---

# Homework and Review

Work through the Session 13 homework exercises available here.

[Homework](homework_numpy.qmd#session-13-loading-and-saving-data){.btn .btn-primary}
