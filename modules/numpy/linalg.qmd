...
NumPy represents vectors and matrices as arrays. We will first review matrix multiplication, solve a linear system, and compute eigenvalues. We will then apply these tools to image compression using singular value decomposition (SVD).
...
::: {.callout-note}
## External Image Activity

The image-compression section downloads images from the internet and is intended to run in Google Colab. The page displays its code without executing it during website rendering.
:::
...
Use `@` for matrix multiplication. The inner dimensions must match.
...
To solve $A\mathbf{x}=\mathbf{b}$, use `np.linalg.solve()` instead of explicitly calculating $A^{-1}\mathbf{b}$.
...
Solve the following system and verify the result with matrix multiplication:
...
An eigenvector keeps its direction when multiplied by its matrix:
...
v = eigenvectors[:, 0]
print("A @ v:", A @ v)
print("lambda * v:", eigenvalues[0] * v)
```

For a real symmetric matrix, `np.linalg.eigh()` is more appropriate than the general `np.linalg.eig()` function and guarantees real eigenvalues.
...
```{python}
# Get the dimensions (height and width) of the dog image
h, w = dog.shape
...
compressed_image = eigenvectors @ np.diag(eigenvalues) @ np.linalg.inv(eigenvectors)

# Display the compressed image
show_image(compressed_image)
```

For a general, non-symmetric image matrix, eigenvalues and eigenvectors may be complex. The reconstruction formula also requires the inverse of the eigenvector matrix, not its transpose. These facts make eigen decomposition an awkward compression tool for a general image.
...
largest = np.argsort(np.abs(eigenvalues))[::-1][:k]
top_k_eigenvalues = eigenvalues[largest]
top_k_eigenvectors = eigenvectors[:, largest]
...
- `np.linalg.solve(A, b)` solves $A\mathbf{x}=\mathbf{b}$ directly.
- Eigenvectors satisfy $A\mathbf{v}=\lambda\mathbf{v}$.
- `eigh()` is designed for real symmetric or complex Hermitian matrices.
- SVD works for rectangular matrices and supports low-rank approximation.

Explain why keeping fewer singular values reduces storage but also removes information from the reconstructed image.

---

# Homework and Review

Work through the Session 18 homework exercises available here.

[Homework](homework_numpy.qmd#session-18-linear-algebra){.btn .btn-primary}
