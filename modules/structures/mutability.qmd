...
`modified_list` is `None` because the function has no `return` statement. `sample_list` is still modified in place, so the last print shows the extra element.
...
`modified_list` is still `None`, but now the original list stays unchanged because the function appends to a copied list instead of the input list.
...
`dict_animals` ends up with four keys, because the default dictionary created for `dict_base` is shared across calls. The animal entries are added to the same dictionary that already contains the clothing entries.
...
The final `st_evens` has `15` elements, because the same default set is reused and grows between the second and third calls.
...
```{python}
def update_dict(key, value, dict_base: dict=None) -> dict:
