...
Let's start by comparing two numeric values. Open a new Python notebook and run the following code snippets to see how comparison operators work. Remember you can copy the code by clicking the "Copy" button at the top right of each code cell; and you can run the code in your notebook by clicking the "Run" button at the top left of each code cell.
...
As you can see in the two examples above, comparisons return either `True` or `False`.
...
Comparison operators care about the type of the values being compared. For instance, a string and a number will never be equal.
...
However, we can compare two strings.
...
The comparison is case-sensitive (lowercase vs. uppercase). Two strings will be considered equal only when they are an exact match.
...
Try the cell above changing the value of `years` to a negative number. The code should not print anything.

What happens if you turn `years` into a string? Try it and see what happens.
...
The first block fails because the `print` line is not indented under the `if` clause. The second block uses `=` instead of `==` and is also missing the colon after the condition. The third block compares a string with a number instead of checking the length of the string.

```{python}
# Prints the square root of a number only if it is positive
x = 4
...
The first block prints `The name does not contain the letter a` because the code looks for lowercase `a`, and `Alberto` only contains uppercase `A`. The second block raises a syntax error because the `else` line is missing a colon. The third block prints `x is positive` because `5 < 0` is `False`, so `y` is `False`.
...
The code compares a string with a number and also uses `=` instead of `==` inside the nested `if` clause.
...
The clause `elif x < -5` will never happen, because every number below `-5` already satisfies the earlier condition `elif x < 0`.
...
When you feel ready, you can test your knowledge by working through the [review exercises](review_flow.qmd).

Work through the "Flow Control" homework exercises available here. To earn participation credit, you must complete the exercises highlighted in red.
