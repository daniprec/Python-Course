...
def parse_qmd_execute_flags(path: Path):
    ...
    for m in FENCE_RE.finditer(text):
        ...
        # header typically: 'python execute=false' or 'python'
        if not header:
            continue
        parts = header.split()
        lang = parts[0].strip()
        if not lang:
            continue
        # search for execute=false or execute:false (case-insensitive)
        execute_false = any(
...
def sync_file(qmd_path: Path, apply: bool):
    ...
    if not flags:
        print(
            f"[OK] No execute=false flags found in {qmd_path.relative_to(Path.cwd())}"
        )
        return
    ...
    for nb_path in nb_paths:
        ...
        for i, cell in enumerate(code_cells):
            ...
            if desired:
                ...
                # remove explicit false to avoid stale settings
